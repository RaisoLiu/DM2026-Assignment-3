#!/usr/bin/env python3
"""Aggressive Generator candidate 2: 5-source weighted ensemble whose weights
were optimized DIRECTLY on HOS-12 (not on the 52-user LNUO).

Weights: v1=0.011, v2=0.588, cm=0.240, c22l=0.075, c22x=0.086
HOS-12 macro: 0.7997 (split halves: 0.7911 / 0.7861 — robust)

This mechanism explicitly trades off OOF generalization for HOS-12 robustness
on the assumption — corroborated by the past 3 days of failures — that the
52-user OOF→public mapping is broken, and HOS-12 is the only reliable proxy.
"""
...
def main() -> None:
    out_csv = ROOT / "submissions/agg_5src_hos12_optimal.csv"
    out_meta = ROOT / "submissions/agg_5src_hos12_optimal_metadata.json"
    ...
    blended_test = (
        weights["ssl_v1"] * p_v1
        + weights["ssl_v2"] * p_v2
        + weights["centered_meta"] * p_cm
        + weights["catch22_lgbm"] * p_c22l
        + weights["catch22_xgb"] * p_c22x
    )
    blended_test = blended_test / blended_test.sum(axis=1, keepdims=True)

    # --- OOF (fit Viterbi transitions on full train) ---
    ...
    meta = {
        "mechanism": "5-source weighted ensemble with weights tuned directly on HOS-12 (NOT LNUO); Viterbi smoothing",
        "untested_on_public": True,
        "weights": weights,
        "alpha": alpha,
        "beta": beta,
        "hos12_macro_f1": 0.7997,
        "hos12_split_half_macros": [0.7911, 0.7861],
        "hos12_per_class_f1": {2: 0.422, 3: 0.712, 4: 0.883, 5: 0.855},
